...
public class Parser {
    public static CasoTeste[] traduz(List<String> linhas) {
        ...
        int n = Integer.parseInt(linhas.get(indice)); // quantos casos de teste
        ...
        for (int i = 0; i < n; i++) {
            ...
            String[] parte = linhas.get(indice).split(" ");
            ...
            for(int j = 0; j < t; j++) {
                parte = linhas.get(indice).split(" "); // substitui as antigas
    ...
    public static int pularLinhasVazias(List<String> linhas, int indice) { // verifica e pula linhas em branco
        while (linhas.get(indice).trim().isEmpty()) {
            indice++;
        }
        return indice; 
    }
